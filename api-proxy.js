...
const EMAIL_FROM = process.env.EMAIL_FROM || 'Your Barbershop <noreply@example.com>';
    ...
    html: `
      <div dir="rtl" style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;background:#1a1a1a;color:#fff;border-radius:12px;overflow:hidden">
        <div style="background:#C5A059;padding:20px;text-align:center">
          <h1 style="margin:0;color:#fff;font-size:22px">✂️ המספרה של דני — מספרת גברים</h1>
        </div>
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/api/push/send' && req.method === 'POST') {
    ...
    readBody(req, res, corsHeaders, async (body) => {
      try {
        const data = JSON.parse(body);
        const payload = {
          title: data.title || 'המספרה של דני',
          body: data.body || '',
  ...
  if (req.url === '/api/barbers' && req.method === 'GET') {
    const fetchUrl = new URL('/rest/v1/barbers?select=id,name,base_price&order=created_at.asc', SUPABASE_URL);
    https.get({
